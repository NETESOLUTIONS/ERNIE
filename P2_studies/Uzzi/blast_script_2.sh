...
python3.7 observed_frequency_r.py $1 $working_directory/${file_name}_observed_frequency.csv
...
do
	...
	python3.7 background_frequency_r.py $filename $number $dir_name/$2/ $working_directory/$2/
	if [ "$?" != 0 ]; then
		exit 1
	fi
...
python3.7 journal_count_r.py $working_directory/$2/ $total $working_directory/${file_name}_observed_frequency.csv
if [ "$?" != 0 ]; then
	exit 1
fi
