...
def generate_table(data_set,number):
    ...
    df_c['reference_pairs']=df_['C']+','+df_['D']
    del(df_)
    ...
    full_list=pd.merge(df_c.iloc[:,2:],data_set,on='reference_pairs',how='inner')
...
data_set.sort_values(by=['source_id','cited_source_uid','reference_issn'],inplace=True)
data_set.reset_index(inplace=True,drop=True)
