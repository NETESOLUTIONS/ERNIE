...
y1 <- cat_d95_sub_cs95 %>% group_by(conventionality,novelty) %>% summarize(count=length(source_id)) %>% data.frame() %>% mutate(percent=round((100*count/dim(cat_d95_sub_cs95)[1]))) %>% mutate(class='all')
z1 <-rbind(z1,y1)
...
z1 <- z1 %>% mutate(bg='d95_sub_cs95')
Z_cs <- rbind(z,z1)

setDT(Z_cs)
temp <- Z_cs[class=='top1'|class=='all']
dodge <- position_dodge(width = 0.5)
pdf('cs95.pdf')
ggplot(temp,aes(gp,percent)) + geom_bar(aes(fill = class), stat="identity",position=dodge, width=0.5)  + facet_grid(bg ~ .) + ggtitle("Cocit_Search95")
dev.off()
