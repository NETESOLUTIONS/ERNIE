...
CREATE INDEX imm1985_testcase_asjc_idx ON theta_plus.imm1985_testcase_asjc2403(scp)
TABLESPACE index_tbs;

CREATE TABLE theta_plus.imm1985_testcase_cited
TABLESPACE theta_plus_tbs AS
SELECT tp.scp as citing,sr.ref_sgr as cited from theta_plus.imm1985_testcase_asjc2403 tp
INNER JOIN scopus_references sr on tp.scp = sr.scp
CREATE INDEX imm1985_testcase_cited_idx ON theta_plus.imm1985_testcase_cited(citing,cited)
TABLESPACE index_tbs
...
CREATE TABLE theta_plus.imm85_nodes
TABLESPACE theta_plus_tbs AS
...
CREATE TABLE theta_plus.imm85_title_abstracts
TABLESPACE theta_plus_tbs AS
SELECT tpin.scp,st.title,sa.abstract_text
FROM theta_plus.imm85_nodes tpin
...
select scp,title from theta_plus.imm85_title_abstracts limit 5;
select count(1) from theta_plus.imm85_title_abstracts;
