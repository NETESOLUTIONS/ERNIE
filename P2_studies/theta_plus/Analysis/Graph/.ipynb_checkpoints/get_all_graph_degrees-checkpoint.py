...
total_deg = pd.DataFrame.from_dict(degrees, orient='index', columns=['total_degrees'])
total_deg['scp'] = total_deg.index
total_deg = total_deg.reset_index(drop=True)

indegrees = dict(G.in_degree())
total_in_deg = pd.DataFrame.from_dict(indegrees, orient='index', columns=['in_degrees'])
total_in_deg['scp'] = total_in_deg.index
total_in_deg = total_in_deg.reset_index(drop=True)

outdegrees = dict(G.out_degree())
total_out_deg = pd.DataFrame.from_dict(outdegrees, orient='index', columns=['out_degrees'])
total_out_deg['scp'] = total_out_deg.index
total_out_deg = total_out_deg.reset_index(drop=True)

scp_all = total_deg.merge(total_in_deg).merge(total_out_deg).sort_values(by=['total_degrees'], ascending=False)
scp_all = scp_all[['scp', 'total_degrees', 'in_degrees', 'out_degrees']]

scp_all.to_sql('imm1985_1995_cluster_degrees', con=engine, schema=schema, if_exists='append', index=False)
print("All Completed.")