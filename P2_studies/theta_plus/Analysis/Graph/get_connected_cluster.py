...
start_num = clusters_list.index(start_cluster_num)
        ...
        for k,v in all_clusters.items():
            if v == max_edges:
                result_dict = {'cluster_no': cluster_num, 
                               'max_edge_match': k, 
                               'max_edges': [v]}
                pd.DataFrame.from_dict(result_dict).to_sql('superset_30_350_connected_clusters', con=engine, schema=schema, if_exists='append', index=False)
