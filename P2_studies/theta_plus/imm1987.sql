...
DROP TABLE IF EXISTS theta_plus.imm1987_testcase_cited;
...
FROM theta_plus.imm1987_citing_cited;
CREATE INDEX imm1990_nodes_idx ON theta_plus.imm1987_nodes(scp);

DROP TABLE IF EXISTS theta_plus.imm1987_title_abstracts;
CREATE TABLE theta_plus.imm1987_title_abstracts
TABLESPACE theta_plus_tbs AS
SELECT tpin.scp,st.title,sa.abstract_text
FROM theta_plus.imm1987_nodes tpin
...
select scp,title from theta_plus.imm1987_title_abstracts limit 5;
select count(1) from theta_plus.imm1987_title_abstracts;
