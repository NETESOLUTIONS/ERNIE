...
DROP TABLE IF EXISTS theta_plus_ecology.eco2000;

CREATE TABLE theta_plus_ecology.eco2000
...
CREATE INDEX eco2000_idx
    ON theta_plus_ecology.eco2000(scp)
TABLESPACE index_tbs;

-- Get all the cited references of the ecology articles published in 2000
DROP TABLE IF EXISTS theta_plus_ecology.eco2000_cited;

CREATE TABLE theta_plus_ecology.eco2000_cited
TABLESPACE theta_plus_tbs AS
SELECT eco.scp AS citing,sr.ref_sgr AS cited
FROM theta_plus_ecology.eco2000 eco
INNER JOIN public.scopus_references sr 
    ON eco.scp = sr.scp;

CREATE INDEX eco2000_cited_idx
    ON theta_plus_ecology.eco2000_cited(citing,cited)
TABLESPACE index_tbs;

-- Get all the citing references of the ecology articles published in 2000
DROP TABLE IF EXISTS theta_plus_ecology.eco2000_citing;

CREATE TABLE theta_plus_ecology.eco2000_citing 
TABLESPACE theta_plus_tbs AS
...
ON theta_plus_ecology.eco2000_citing(citing,cited)
TABLESPACE index_tbs;

-- Create table from the union of cited and citing references
DROP TABLE IF EXISTS theta_plus_ecology.eco2000_citing_cited;

CREATE TABLE theta_plus_ecology.eco2000_citing_cited
TABLESPACE theta_plus_tbs AS
SELECT DISTINCT citing,cited from theta_plus_ecology.eco2000_cited 
UNION
SELECT DISTINCT citing,cited from theta_plus_ecology.eco2000_citing;


-- clean up Scopus data
DELETE FROM theta_plus_ecology.eco2000_citing_cited
WHERE citing=cited;
...
ALTER TABLE theta_plus_ecology.eco2000_citing_cited
RENAME TO XX_eco2000;

CREATE TABLE theta_plus_ecology.eco2000_citing_cited AS
...
DROP TABLE IF EXISTS theta_plus_ecology.eco2000_nodes;

CREATE TABLE theta_plus_ecology.eco2000_nodes
TABLESPACE theta_plus_tbs AS
SELECT DISTINCT citing AS scp
FROM theta_plus_ecology.eco2000_citing_cited
UNION
SELECT DISTINCT cited
FROM theta_plus_ecology.eco2000_citing_cited;

CREATE INDEX eco2000_nodes_idx 
    ON theta_plus_ecology.eco2000_nodes(scp);
