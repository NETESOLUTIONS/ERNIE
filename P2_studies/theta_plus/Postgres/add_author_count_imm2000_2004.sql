...
This script is particularly written for the imm2000_2004 table

*/

SET search_path = theta_plus;

-- Add author count info for the imm2000_2004 table

-- region: add columns
ALTER TABLE imm2000_2004_all_merged_unshuffled
...
UPDATE imm2000_2004_all_merged_unshuffled
...
FROM imm2000_2004_cluster_scp_list_unshuffled cslu
LEFT JOIN public.scopus_authors sa
  ON cslu.scp = sa.scp
GROUP BY cslu.cluster_no, sa.auid) authors
GROUP BY authors.cluster_no) percentile_val

WHERE imm2000_2004_all_merged_unshuffled.cluster_no = percentile_val.cluster_no
;
...
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = authors.cluster_no
;
-- end region
...
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = all_authors.cluster_no;
...
UPDATE imm2000_2004_all_merged_unshuffled
 ...
 FROM
    ...
    FROM
            ...
            LEFT JOIN imm2000_2004_all_merged_unshuffled amu
...
FROM
        ...
        JOIN
                    ...
                    FROM (SELECT cslu.cluster_no, sa.auid, count(sa.auid) num_articles
                     ...
                     JOIN imm2000_2004_all_merged_unshuffled amu
                        ON authors.cluster_no = amu.cluster_no
                    WHERE authors.num_articles >= amu.p_50_author_count ) authors_50

            ON scps.auid = authors_50.auid and scps.cluster_no = authors_50.cluster_no
        GROUP BY scps.cluster_no) articles_50
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = articles_50.cluster_no;
...
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = articles_75.cluster_no;
...
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = articles_95.cluster_no;
...
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = articles_99.cluster_no;
...
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = coauthor.cluster_no;
...
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = dist_coauthors.cluster_no;
...
FROM
      ...
      FROM imm2000_2004_all_merged_unshuffled ) prop
WHERE imm2000_2004_all_merged_unshuffled.cluster_no = prop.cluster_no;
