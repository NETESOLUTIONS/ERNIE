...
"""
DROP TABLE IF EXISTS rated_set;
CREATE TABLE rated_set AS
SELECT expert_rating, imm1985_1995_cluster_no, s.current_cluster_size AS imm1985_1995_cluster_size, 
       s.match_year, s.match_cluster_no mcl_year_cluster_no
FROM expert_ratings
JOIN superset_to_year_match_30_350 s
  ON expert_ratings.imm1985_1995_cluster_no = s.current_cluster_number;
"""

main_table_name = 'rated_set'
main_table = pd.read_sql_table(main_table_name, schema=schema, con=engine)
...
print(f'Working on table: {schema}.{main_table_name}')
for i in range(len(main_table)):
    ...
    graclus_half_mclsize_query = "SELECT conductance, int_edges, boundary, sum_article_score, max_article_score, median_article_score FROM theta_plus." + graclus_half_mclsize_table_name + " WHERE cluster_no=" + graclus_half_mclsize_cluster_no + ";"
    ...
    main_table.at[i,'graclus_half_mclsize_coherence'] = None
...
main_table.to_sql(main_table_name, schema=schema, con=engine, if_exists='replace', index=False)
print("All completed.")