...
def generate_obs_freq(df,slice_num):
	...
	df=df.groupby(['source_id'])['s_cited_source_uid'].apply(list).values
	...
	df['reference_pairs']=df['A']+','+df['B']

	#Getting the aggregated count of each journal pair
	df=df['reference_pairs'].value_counts()
	df=pd.DataFrame({'reference_pairs':df.index,'frequency':df.values})
...
fields=['source_id','s_cited_source_uid']
...
data_set.sort_values(by=['source_id','s_cited_source_uid'],inplace=True)
...
data_set=data_set.groupby(by=['reference_pairs'],as_index=False)['frequency'].sum()
