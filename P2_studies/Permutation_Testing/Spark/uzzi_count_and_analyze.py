...
update_mean_udf = udf(lambda x: update_mean(Decimal(x[0]),Decimal(x[1]),Decimal(x[2])), sql_type.DecimalType())
welford_pass_udf = udf(lambda x: welford_pass(Decimal(x[0]),Decimal(x[1]),Decimal(x[2]),Decimal(x[3])), sql_type.DecimalType())
std_udf = udf(lambda x: calculate_std_dev(Decimal(x[0]),Decimal(x[1]),0.0), sql_type.DecimalType()) ##TODO: adjust this based on whether team wants population or sample STDDEV
...
def calculate_journal_pairs_freq(input_dataset,i):
    ...
    print(spark.sql("SELECT * FROM update_table_finished").show()) # test
    df=spark.sql('''
            SELECT  a.journal_pair_A,
                    a.journal_pair_B,
                    a.obs_frequency,
    ...
    print(spark.sql("SELECT * FROM temp_observed_frequencies").show())
