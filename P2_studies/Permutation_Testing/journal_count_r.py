...
obs_file.columns=['reference_pairs','obs_frequency']
pattern='*freq*'
...
for i in range(0,number):
    ...
    obs_file=pd.merge(obs_file,data,on=['reference_pairs'],how='left')
    
print('\n')
#obs_file.to_csv(bg_files+'combined.csv',index=False)

#Renaming column names
column_names=['reference_pairs','obs_frequency']
...
obs_file[['reference_pairs','obs_frequency','mean','z_scores','count']].dropna().to_csv(bg_files+'zscores_file.csv',index=False)
