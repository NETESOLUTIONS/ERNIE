...
rm(list=setdiff(ls(), c("cat_imm95","cat_metab95","cat_d95_sub_imm", "cat_d95_sub_metab", "cat_d95")))
...
z_d95 <- z_d95 %>% mutate(bg=' d95')
z_d95 <- z_d95 %>% arrange(class)
z_d95$class <- factor(z_d95$class, levels=c('all','top10','top9','top8','top7','top6','top5','top4','top3','top2','top1'))
...
d95 <- d95 %>% mutate(gp='wos95')


fig_95 <- rbind(imm95,imm_d95,metab95,metab_d95,d95)
fig_95$class <- factor(fig_95$class,levels=c('all','top10','top9','top8','top7','top6','top5','top4','top3','top2','top1'))
fig_95$gp <- factor(fig_95$gp,levels=c("imm95","metab95","imm_wos95","metab_wos95","wos95"))
fwrite(fig_95,file='fig_95.csv',row.names=FALSE)
...
qplot(class, value, group=gp, color=gp, data=fig_95, geom=c('line','point'),facets=variable~.,xlab="Citation Count Percentile Group",ylab="Percent of Publications") + theme(text=element_text(size=10,  family="Times New Roman"))
dev.off()
