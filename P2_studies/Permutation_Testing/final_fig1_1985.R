...
library(ggplot2)
library(reshape2)
setwd('~/Desktop/Old_Macbook/disciplinary_references/')
...
pdf('background-effect_final.pdf')
qplot(bg, log(value), data=X, facets=algorithm~. ,geom='boxplot',group=bg,color=bg, ylab=expression(paste((log[2]),' fold change in subject frequency of references')),xlab='network background')  + theme_bw() + theme(strip.text.y = element_text(size = 14)) + theme(legend.position = 'none') + My_Theme
dev.off()
rm(list=ls())
