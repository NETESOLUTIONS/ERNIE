...
y[connected=='t',connected:='True']
y[connected=='f',connected:='False']
colnames(y)[8] <- "Connected"
...
theme(text=element_text(family="Arial", size=20)) + theme(axis.text.x=element_text(angle = -75, hjust = 0)) +
theme(legend.text=element_text(size=18))
dev.off()
system('cp lag.pdf ~/cocitation_2/R1/R1lag3.pdf')
