...
CREATE INDEX garfield_dmet_begin_idx on garfield_dmet_begina(target);

-- Get second generation of cited references
DROP TABLE IF EXISTS garfield_dmet_twog_a;
CREATE TABLE garfield_dmet_twog_a AS
SELECT a.target AS source, 'source'::varchar(10) AS stype, b.cited_source_uid as target,'target'::varchar(10) as ttype
FROM garfield_dmet_begin a INNER JOIN wos_references b ON a.target=b.source_id;
