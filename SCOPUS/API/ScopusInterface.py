...
abstract_retrieval_header="https://api.elsevier.com/content/abstract/scopus_id/"
...
def abstract_retrieval(scopus_id, api_key, view='FULL', query_params=deepcopy(query_params_base)):
    query_params.update({'apiKey':api_key, 'view':view})
    document_html=abstract_retrieval_header+"{}?".format(scopus_id)+urllib.urlencode(query_params)
...
def citing_scopus_id_search(scopus_id,api_key,low_bound,high_bound,query_params=deepcopy(query_params_base)):
    ...
    for period in ["{}-{}".format(i,i+1) for i in range (low_bound,high_bound)]:
        ...
        try:
            ...
            citing_scopus_ids=set(citing_scopus_ids); process_cited_by_count=len(citing_scopus_ids)
        except:
...
def author_retrieval(author_id, api_key, view="ENHANCED",query_params=deepcopy(query_params_base)):
    ...
    #if "author_id:" in alias_author_id:
        #data_dict['alias_author_id']= alias_author_id.split("author_id:")[1]
...
def document_search(query,api_key, field="title,identifier",query_params=deepcopy(query_params_base)):
    ...
    data_dict['documents']=set(data_dict['documents']); data_dict['process_document_count']=len(data_dict['documents'])
    return data_dict
