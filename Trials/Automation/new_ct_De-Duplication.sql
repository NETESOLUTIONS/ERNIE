...
CREATE UNIQUE INDEX new_ct_arm_groups_uk
  ON new_ct_arm_groups (nct_id, arm_group_label, arm_group_type, description)
TABLESPACE ct_index_tbs;
-- 4.9s
-- endregion

-- region new_ct_authorities
ALTER TABLE new_ct_authorities
  ADD CONSTRAINT new_ct_authorities_pk PRIMARY KEY (nct_id) USING INDEX TABLESPACE ct_index_tbs;
-- endregion
...
ALTER TABLE new_ct_collaborators
  ADD CONSTRAINT new_ct_collaborators_pk PRIMARY KEY (nct_id, agency) USING INDEX TABLESPACE ct_index_tbs;
-- 1.3s
-- endregion
...
ALTER TABLE new_ct_conditions
  ADD CONSTRAINT new_ct_conditions_pk PRIMARY KEY (nct_id, condition) USING INDEX TABLESPACE ct_index_tbs;
-- 6.4s
-- endregion
...
ALTER TABLE new_ct_condition_browses
  ADD CONSTRAINT new_ct_condition_browses_pk PRIMARY KEY (nct_id, mesh_term) USING INDEX TABLESPACE ct_index_tbs;
-- 7.6s
-- endregion
...
DO $$ BEGIN
  IF NOT EXISTS(SELECT 1
                ...
                WHERE
                  index_pc.relname = 'new_ct_interventions_uk')
  THEN
    CREATE UNIQUE INDEX new_ct_interventions_uk
...
DELETE
FROM
  new_ct_intervention_other_names ction1
WHERE EXISTS(SELECT 1
...
DO $$ BEGIN
  ...
  THEN
    ...
    USING INDEX TABLESPACE ct_index_tbs;
    -- 1.8s
  END IF;
END $$;
-- endregion
...
DELETE
FROM
  new_ct_intervention_browses ctib1
WHERE EXISTS(SELECT 1
...
DELETE
FROM
  new_ct_intervention_arm_group_labels ctiagl1
WHERE EXISTS(SELECT 1
...
CREATE UNIQUE INDEX new_ct_links_uk
...
DO $$ BEGIN
  IF NOT EXISTS(SELECT 1
                FROM pg_index pi
                  JOIN pg_class index_pc ON index_pc.oid = pi.indexrelid
                WHERE
                  index_pc.relname = 'new_ct_locations_uk')
  THEN
    CREATE UNIQUE INDEX new_ct_locations_uk
...
DELETE
FROM
  new_ct_location_investigators ctli1
WHERE EXISTS(SELECT 1
...
DO $$ BEGIN
  IF NOT EXISTS(SELECT 1
                FROM pg_index pi
                  JOIN pg_class index_pc ON index_pc.oid = pi.indexrelid
                WHERE
                  index_pc.relname = 'new_ct_outcomes_uk')
  THEN
...
DELETE
FROM
  new_ct_overall_officials ctoo1
WHERE EXISTS(SELECT 1
...
DO $$ BEGIN
  IF NOT EXISTS(SELECT 1
                FROM pg_index pi
                  JOIN pg_class index_pc ON index_pc.oid = pi.indexrelid
                WHERE
                  index_pc.relname = 'new_ct_overall_officials_uk')
  THEN
...
CREATE UNIQUE INDEX new_ct_references_uk
