...
\set ECHO all
\set TOTAL_NUM_ASSERTIONS 47 -- However, Jenkins can run tests without plan,  but serves a good indicator of the number of affirmations
...
FOR tab IN
 ...
 END LOOP;
  RETURN NEXT is_empty( 'select tablename, attname from pg_stats
   where schemaname = ''public'' and tablename LIKE ''ct%'' and null_frac = 1', 'No 100% null column');
END;
$$ LANGUAGE plpgsql;
