...
SELECT
  pid AS process_id, usename AS username, client_addr AS client_address, application_name, backend_start, state,
  state_change
...
SELECT
  ...
  FROM
    pg_locks blocked_pl
      ...
      JOIN pg_locks blocking_pl
    ON (((blocking_pl.transactionid = blocked_pl.transactionid) OR (blocking_pl.relation = blocked_pl.relation
        AND blocking_pl.locktype = blocked_pl.locktype)) AND blocked_pl.pid != blocking_pl.pid)
      JOIN pg_stat_activity blocking_psa
    ON blocking_pl.pid = blocking_psa.pid AND blocking_psa.datid = blocked_psa.datid
