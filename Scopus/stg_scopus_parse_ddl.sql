...
create or replace procedure jenkins.stg_scopus_parse_abstracts_and_titles(scopus_doc_xml xml)
	language plpgsql
as $$
BEGIN
    -- scopus_abstracts
...
alter procedure jenkins.stg_scopus_parse_abstracts_and_titles(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_authors_and_affiliations(scopus_doc_xml xml)
	language plpgsql
as $$
BEGIN
    -- scopus_authors
...
alter procedure jenkins.stg_scopus_parse_authors_and_affiliations(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_chemical_groups(scopus_doc_xml xml)
	language plpgsql
as $$
DECLARE
    cur RECORD;
...
alter procedure jenkins.stg_scopus_parse_chemical_groups(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_grants(scopus_doc_xml xml)
	language plpgsql
as $$
BEGIN
    -- scopus_grants
...
alter procedure jenkins.stg_scopus_parse_grants(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_keywords(scopus_doc_xml xml)
	language plpgsql
as $$
BEGIN
  INSERT
...
alter procedure jenkins.stg_scopus_parse_keywords(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_pub_details_subjects_and_classes(scopus_doc_xml xml)
	language plpgsql
as $$
BEGIN
...
alter procedure jenkins.stg_scopus_parse_pub_details_subjects_and_classes(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_publication_and_group(scopus_doc_xml xml)
	language plpgsql
as $$
DECLARE
    cur RECORD;
...
alter procedure jenkins.stg_scopus_parse_publication_and_group(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_references(input_xml xml)
	language plpgsql
as $$
DECLARE
    v_state   TEXT;
...
alter procedure jenkins.stg_scopus_parse_references(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_references_one_by_one(input_xml xml)
	language plpgsql
as $$
DECLARE row RECORD;
  BEGIN
...
alter procedure jenkins.stg_scopus_parse_references_one_by_one(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_source_and_conferences(scopus_doc_xml xml)
	language plpgsql
as $$
...
--         RETURNING ernie_source_id into db_id;
        COMMIT;
    -- scopus_isbns
    INSERT INTO stg_scopus_isbns(isbn, isbn_length, isbn_type, isbn_level)
    SELECT DISTINCT
           isbn,
           isbn_length,
    ...
    FROM xmltable(--
                ...
                isbn_level TEXT PATH '@level'
                                                      ...
                                                      COMMIT;
    -- scopus_issns
    INSERT INTO stg_scopus_issns( issn, issn_type)
    SELECT
           issn,
    ...
    INSERT INTO stg_scopus_conf_proceedings(conf_code, conf_name, proc_part_no, proc_page_range,
                                        proc_page_count)
    SELECT
    ...
    COMMIT;
    -- scopus_conf_editors
    INSERT INTO stg_scopus_conf_editors( conf_code, conf_name, indexed_name, role_type,
...
alter procedure jenkins.stg_scopus_parse_source_and_conferences(xml) owner to djamil;

create or replace procedure jenkins.stg_scopus_parse_publication_identifiers(scopus_doc_xml xml)
	language plpgsql
as $$
BEGIN
