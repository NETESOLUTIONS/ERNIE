...
DO $block$
  ...
  BEGIN
    ...
    IF current_setting('script.subset_sp')='' THEN -- Execute all parsing SPs
      ...
      CALL jenkins.stg_scopus_parse_source_and_conferences(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_pub_details_subjects_and_classes(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_authors_and_affiliations(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_chemical_groups(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_abstracts_and_titles(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_keywords(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_publication_identifiers(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_grants(scopus_doc_xml);
      CALL jenkins.stg_scopus_parse_references(scopus_doc_xml);
