...
BEGIN
  ...
  INSERT INTO stg_scopus_affiliations(scp, affiliation_no, afid, dptid, city_group, state, postal_code, country_code,
                                      country)
  SELECT
    scp, affiliation_no, afid, dptid, coalesce(city_group, city) AS city_group, state, postal_code, country_code,
    country
    FROM
      xmltable(--
        ...
        SELECT scp, string_agg(organization, ',') AS organization
          FROM
            xmltable(--
              ...
              scp BIGINT PATH '../../../preceding-sibling::item-info/itemidlist/itemid[@idtype="SCP"]', organization TEXT PATH 'normalize-space()')
         GROUP BY scp
      ) AS sq
   WHERE sa.scp = sq.scp;
  ...
  SELECT DISTINCT t1.scp, t1.affiliation_no, t2.author_seq
    FROM
      xmltable(--
          '//bibrecord/head/author-group/affiliation' PASSING scopus_doc_xml COLUMNS --
        scp BIGINT PATH '../../preceding-sibling::item-info/itemidlist/itemid[@idtype="SCP"]', affiliation_no FOR ORDINALITY , afid BIGINT PATH '@afid') AS t1,

      xmltable(--
          '//bibrecord/head/author-group/author' PASSING scopus_doc_xml COLUMNS --
        author_seq SMALLINT PATH '@seq', afid BIGINT PATH '../affiliation/@afid') AS t2
