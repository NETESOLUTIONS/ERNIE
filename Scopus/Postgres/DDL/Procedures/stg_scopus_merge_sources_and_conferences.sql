...
BEGIN
  -- TODO Looping looks unnecessary
  FOR single_row IN (
    SELECT DISTINCT
      ernie_source_id, source_id, issn_main, isbn_main, source_type, source_title, coden_code, website, publisher_name,
      publisher_e_address, pub_date
  ...
  INSERT INTO scopus_conf_proceedings(ernie_source_id, conf_code, conf_name, proc_part_no, proc_page_range,
                                      proc_page_count)
  SELECT DISTINCT
    scopus_sources.ernie_source_id, conf_code, conf_name, string_agg(DISTINCT proc_part_no, ','), max(proc_page_range),
    max(proc_page_count)
  ...
  SELECT DISTINCT
    scopus_sources.ernie_source_id, conf_code, conf_name, indexed_name, surname, degree, address, organization
