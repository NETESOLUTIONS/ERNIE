...
BEGIN
  ...
  SELECT
    scopus_publications.scp, grant_id, max(grantor_acronym) AS grantor_acronym, grantor,
    max(grantor_country_code) AS grantor_country_code, max(grantor_funder_registry_id) AS grantor_funder_registry_id
  ...
  SELECT DISTINCT scopus_publications.scp, grant_text
    FROM stg_scopus_grant_acknowledgments, scopus_publications
