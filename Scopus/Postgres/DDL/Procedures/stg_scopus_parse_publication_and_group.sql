...
BEGIN
  ...
  FOR cur IN (
    SELECT
      sgr, pub_year, scp, correspondence_person_indexed_name, correspondence_city, correspondence_country,
      correspondence_e_address, citation_type, citation_language
      FROM
        xmltable(--
        -- The `xml:` namespace doesn't need to be specified
          ...
          -- noramlize-space() converts NULLs to empty strings
              correspondence_person_indexed_name TEXT PATH 'bibrecord/head/correspondence/person/ce:indexed-name[1]', --
              correspondence_city TEXT PATH 'bibrecord/head/correspondence/affiliation/city[1]', --
              correspondence_country TEXT PATH 'bibrecord/head/correspondence/affiliation/country[1]', --
              correspondence_e_address TEXT PATH 'bibrecord/head/correspondence/ce:e-address[1]', --
    ...
    INSERT INTO stg_scopus_publications(scp, sgr, correspondence_person_indexed_name, correspondence_city,
                                        ...
                                        citation_language)
    VALUES
      (cur.scp, cur.sgr, cur.correspondence_person_indexed_name, cur.correspondence_city,
