...
BEGIN
  FOR tab IN
  (SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ('scopus_abstracts','scopus_authors','scopus_grants', 'scopus_grant_acknowledgments',
  ...
  RETURN NEXT is_empty( 'select distinct tablename, attname from pg_stats
   where schemaname = ''public'' and tablename = ('scopus_abstracts','scopus_authors','scopus_grants', 'scopus_grant_acknowledgments',
