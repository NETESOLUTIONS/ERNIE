...
BEGIN
  FOR tab IN
  (SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND select table_name in ('scopus_abstracts','scopus_authors','scopus_grants', 'scopus_grant_acknowledgments',
  ...
  RETURN NEXT is_empty( 'select distinct tablename, attname from pg_stats
   where schemaname = ''public'' and select tablename in ('scopus_abstracts','scopus_authors','scopus_grants', 'scopus_grant_acknowledgments',
