...
do
  ...
  wos_non_ref_update_process_start=`date +%s`

  # Update wos tables other than wos_referneces.
  psql -d ernie -f wos_update_tables.sql
  ...
  echo $((wos_non_ref_update_process_start-tar_gz_process_start)) | awk '{print  int($1/60)":"int($1%60)  " : Single CORE File Split-Parse-Load-Preparation Duration UTC" }' >> time_keeper.txt
  echo $((wos_ref_update_process_start-wos_non_ref_update_process_start)) | awk '{print  int($1/60)":"int($1%60)  " : Single CORE File wos_except_ref update processing Duration UTC" }' >> time_keeper.txt
  echo $((wos_ref_update_process_end-wos_ref_update_process_start)) | awk '{print  int($1/60)":"int($1%60)  " : Single CORE File wos_ref processing Duration UTC" }' >> time_keeper.txt
  echo $((tar_gz_process_end-tar_gz_process_start)) | awk '{print  int($1/60)":"int($1%60)  " : Single CORE File TOTAL processing Duration UTC" }' >> time_keeper.txt
...
psql -d ernie -f wos_delete_tables.sql -v delete_csv="'"$c_dir"del_wosid.csv'"

delete_process_end=`date +%s`

echo $((delete_process_end-delete_process_start)) | awk '{print  int($1/60)":"int($1%60)  " : All Delete File processing Duration UTC" }' >> time_keeper.txt
...
# Send log via email.
psql -d ernie -c 'select * from update_log_wos order by id;' | mail -s "WOS annual load test Log" samet@nete.com

process_end=`date +%s`
echo $((process_end-process_start)) | awk '{print  int($1/60)":"int($1%60) " : UPDATE PROCESS END-to-END  DURATION UTC"}' >> time_keeper.txt
