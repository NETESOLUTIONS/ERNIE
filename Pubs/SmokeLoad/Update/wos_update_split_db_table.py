...
Note: 1. To load csv data to each subtable, you need to run a seperate command in shell afterwards:
         psql -d pardi -f load_csv_table.sql
...
def create_table(cur, input_tablename, current_tablename):
  ...
  try:
    cur.execute("""CREATE TABLE public.%s TABLESPACE wos_load_tbs AS SELECT * FROM  %s  LIMIT 0;""",
...
if __name__ == '__main__':
    ...
    #csv_dir = '/pardidata1/split_table_csv/'
    ...
    try:
       conn=psycopg2.connect("dbname='pardi' user='samet'")
    except:
       print "I am unable to connect to the database pardi."
