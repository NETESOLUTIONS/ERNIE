...
--     wos_references
-- Usage: psql -d pardi -f wos_update_ref_tables.sql
...
create index new_ref_chunk_idx on :new_ref_chunk
  using btree (source_id, cited_source_uid) tablespace wos_load_index_tbs;

-- Create a temp table to store WOS IDs from the update file.
drop table if exists temp_update_ref_wosid;
create table temp_update_ref_wosid tablespace wos_load_tbs as
...
create table temp_replace_ref_wosid tablespace wos_load_tbs as
...
--create table temp_wos_reference_1 tablespace wos_load_tbs as
