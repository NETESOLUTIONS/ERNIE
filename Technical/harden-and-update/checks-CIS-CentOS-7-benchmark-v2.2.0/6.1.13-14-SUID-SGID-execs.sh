...
coproc { df --local --output=target \
  | tail -n +2 \
  | xargs -I '{}' find '{}' ${exclude_dir_option} -xdev -type f -perm "/u=s,g=s" -print;
}
# As of Bash 4, `COPROC_PID` has to be saved before it gets reset on process termination
_co_pid=$COPROC_PID
while IFS= read -r file; do
  if [[ -u "$file" ]] && ! grep -F --line-regexp "--file=${SUID_WHITELIST}" || \
     [[ -g "$file" ]] && ! grep -F --line-regexp "--file=${SGID_WHITELIST}"; then
