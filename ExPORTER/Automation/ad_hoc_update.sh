# EXPORTER Project manual download and update
...
wget -q "$PRJLINK" --no-check-certificate
if [ $? -ne 0 ]; then
  echo "Error: No CSV Download Available for year : $PRJLINK";
  echo "Please perform a manual check of the Exporter website and manually update counter file if necessary"
  exit 1;
fi
wget -q "$PRJABSLINK" --no-check-certificate
...
psql ernie -c "INSERT INTO exporter_project_abstracts 
              SELECT * from temp_exporter_project_abstracts 
              ON CONFLICT ON CONSTRAINT exporter_project_abstracts_pk 
              DO UPDATE SET APPLICATION_ID=excluded.APPLICATION_ID, ABSTRACT_TEXT=excluded.ABSTRACT_TEXT;"
